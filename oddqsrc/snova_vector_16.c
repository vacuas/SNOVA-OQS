...
/**
 * Optimized implementation that uses vectorization instructions for AVX2 and NEON.
 *
...
static VECTOR vector_inv_table;
...
static void init_vector_table(void) {
	alignas(32) uint8_t mask[4][32];
	alignas(32) uint8_t multtab256[4][32];

	int idx = 1;
	for (int i1 = 0; i1 < 4; i1++) {
		for (int j1 = 0; j1 < 32; j1++) {
			mask[i1][j1] = idx;
			multtab256[i1][j1] = gf_multtab[idx * SNOVA_q + j1 % 16];
		}
		idx *= 2;
	}

	vtl_multmask1 = *(VECTOR *)mask[0];
	vtl_multmask2 = *(VECTOR *)mask[1];
	vtl_multmask4 = *(VECTOR *)mask[2];
	vtl_multmask8 = *(VECTOR *)mask[3];

	vtl_mult_table1 = *(VECTOR *)multtab256[0];
	vtl_mult_table2 = *(VECTOR *)multtab256[1];
	vtl_mult_table4 = *(VECTOR *)multtab256[2];
	vtl_mult_table8 = *(VECTOR *)multtab256[3];

	// GF16 inverse table
	alignas(32) uint8_t inv_table[32] = {0, 1, 9, 14, 13, 11, 7, 6, 15, 2, 12, 5, 10, 4, 3, 8,
	                                     0, 1, 9, 14, 13, 11, 7, 6, 15, 2, 12, 5, 10, 4, 3, 8
	                                    };
	vector_inv_table = *(VECTOR *)inv_table;
}

// Constant time shuffle table
static VECTOR vtl_ct_multtab(uint8_t val) {
	VECTOR val256 = VEC_DUP(val);
...
static uint16_t ct_gf_inverse(uint16_t val) {
	VECTOR g256 = VEC_DUP(val);
	VECTOR res256 = VEC_SHUFFLE(vector_inv_table, g256);
	return VEC_BYTE0(res256);
}

/**
 * S matrix, prepared expanded.
 */
...
#define GAUSS_ROW (SNOVA_m * SNOVA_l2 + 1)
#define GAUSS_ROW32 ((GAUSS_ROW + VEC_LENGTH - 1) / VEC_LENGTH)
#define SNOVA_ml2 (GAUSS_ROW32 * VEC_LENGTH)

/**
 * Optimized version of Sign. Deterministic using the salt provided
 *
 * The vectorization instructions (both AVX2 and NEON) are only used in the Gaussian elimination step.
 */
int SNOVA_NAMESPACE(sign)(const expanded_SK *skx, uint8_t *sig, const uint8_t *digest, const size_t len_digest,
	...
	do {
		...
		/**
		 * Vectorized Gaussian elimination in constant time
		 */
		alignas(VEC_LENGTH) uint8_t Gauss[SNOVA_m * SNOVA_l2][SNOVA_ml2];

		for (int ti = 0; ti < SNOVA_m * SNOVA_l2; ++ti)
			for (int tj = 0; tj < SNOVA_ml2; ++tj) {
				Gauss[ti][tj] = gf16_compress(gauss16[ti][tj]);
			}

		for (int mi2 = 0; mi2 < SNOVA_m * SNOVA_l2; ++mi2) {
			int swap = ct_is_not_zero(Gauss[mi2][mi2]) - 1;
			for (int j2 = mi2 + 1; j2 < SNOVA_m * SNOVA_l2; ++j2) {
				VECTOR swap256 = VEC_DUP(swap);
				...
				swap = ct_is_not_zero(Gauss[mi2][mi2]) - 1;
			}
			flag_redo |= swap;

			// Constant time GF16 inverse
			VECTOR t_256 = vtl_ct_multtab(ct_gf_inverse(Gauss[mi2][mi2]));

			int kstart = (mi2 / VEC_LENGTH) * VEC_LENGTH;
			for (int k = kstart; k < SNOVA_ml2; k += VEC_LENGTH) {
				*(VECTOR *)(Gauss[mi2] + k) = VEC_SHUFFLE(t_256, *(VECTOR *)(Gauss[mi2] + k));
			}

			for (int j2 = mi2 + 1; j2 < SNOVA_m * SNOVA_l2; ++j2) {
				t_256 = vtl_ct_multtab(Gauss[j2][mi2]);
				for (int k2 = kstart; k2 < SNOVA_ml2; k2 += VEC_LENGTH) {
					*(VECTOR *)(Gauss[j2] + k2) ^= VEC_SHUFFLE(t_256, *(VECTOR *)(Gauss[mi2] + k2));
				}
			}
		}
