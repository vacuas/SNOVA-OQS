...
// Caveat: There are more efficient ways of making SNOVA fully constant time where "constant time" is defined as: passes the
// valgrind test. #define vtl_multtab(x) (vtl_ct_multtab(x & 0xf) ^ (vtl_ct_multtab(x >> 4) << 4))
...
void calc_LR_J_vtl(uint8_t L_J_nibble[m_SNOVA][alpha_SNOVA][rank_next2 / 2][vtl_v_len * VEC_LENGTH],
                   uint8_t R_tr_J[m_SNOVA][alpha_SNOVA][rank_next2][vtl_v_len * VEC_LENGTH],
                   uint8_t R_tr_J_nibble[m_SNOVA][alpha_SNOVA][rank_next2 / 2][vtl_v_len * VEC_LENGTH], Aalpha_t Aalpha,
                   Balpha_t Balpha, Qalpha1_t Qalpha1, Qalpha2_t Qalpha2, gf16m_t *X_in_GF16Matrix) {
	...
	for (int mi = 0; mi < m_SNOVA; ++mi) {
		...
		for (int alpha = 0; alpha < alpha_SNOVA; alpha += 2) {
			...
			for (int ni = 0; ni < rank; ++ni) {
				for (int nj = 0; nj < rank; ++nj) {
					VECTOR k1_lh =
					    mtk2_16[get_gf16m(Aalpha_vtl[alpha], ni, nj) | (get_gf16m(Aalpha_vtl[alpha + 1], ni, nj) << 4)];
					VECTOR k2_lh =
					    mtk2_16[get_gf16m(Qalpha2_vtl[alpha], ni, nj) | (get_gf16m(Qalpha2_vtl[alpha + 1], ni, nj) << 4)];
		...
		for (int alpha = 0; alpha < alpha_SNOVA; ++alpha) {
			...
			for (int ni = 0; ni < rank_floor2; ni += 2) {
				...
				for (int nj = 0; nj < rank; ++nj) {
					VECTOR k1_lh =
					    mtk2_16[get_gf16m(Qalpha1[mi][alpha], ni, nj) ^ (get_gf16m(Qalpha1[mi][alpha], ni + 1, nj) << 4)];
					VECTOR k2_lh =
					    mtk2_16[get_gf16m(Balpha[mi][alpha], nj, ni) ^ (get_gf16m(Balpha[mi][alpha], nj, ni + 1) << 4)];
					for (int nk = 0; nk < vtl_v_len; ++nk) {
 ...
 * Computes signature
 */
int sign_digest_core_gnl_vtl(uint8_t *pt_signature, const uint8_t *digest, uint64_t bytes_digest, const uint8_t *array_salt,
...
int verify_signture_vtl_core(const uint8_t *pt_digest, uint64_t bytes_digest, const uint8_t *pt_signature,
...
int SNOVA_NAMESPACE(verify)(const uint8_t *pt_digest, size_t bytes_digest, const uint8_t *pt_signature, const uint8_t *pk) {
	snova_init();
	return verify_core(pt_digest, (uint64_t)bytes_digest, pt_signature, pk);
}
