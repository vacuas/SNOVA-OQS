 ...
 *
 * The reference implementation of SNOVA-SHAKE can be tested against the regular version using the KAT files.
 * make P="-D REFERENCE_SHAKE" clean kat
 *
 * To use AES-CTR as defined for SNOVA Rounds 1 and 2
 * make P="-D AESCTR" clean kat
 *
...
/**
 * SNOVA public key expander, snova_shake
 */

#ifdef REFERENCE_SHAKE
...
// Single invocation version only to test optimized against reference
